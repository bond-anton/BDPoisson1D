...
from BDPoisson1D import dirichlet_first_order_solver_arrays, dirichlet_first_order_solver_arrays2
...
nodes = np.linspace(start, stop, num=201, endpoint=True)  # generate nodes
...
# result = dirichlet_first_order_solver_arrays(nodes, p_nodes, f_nodes, bc1, j=1.0)  # solve Poisson equation
result = dirichlet_first_order_solver_arrays2(nodes, p_nodes, f_nodes, bc1, bc2, j=1.0)  # solve Poisson equation
...
ax1.plot(nodes, dy_solution + np.asarray(result[:, 0]) * np.asarray(p_nodes), 'b-', label='dy/dx (solution)')
ax1.legend()
